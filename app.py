...
def llmresponse(form_input, sender, recipient, style):
    ...
    """
    llm = CTransformers(model="llm/llama-2-7b-chat.ggmlv3.q8_0.bin", 
                        model_type="llama", 
                        config={"max_new_tokens": 256, "temperature": 0.0})

    template = """
    Write an email with {style} style to {recipient} from {sender} and remember to include topic: {topic}.
    Senders name is {sender} and Recipients name is {recipient}.
    Email text:
    """
...
with col5:
    style = st.selectbox("Writing style", ("formal", "informal", "Satisfied", "Unsatisfied"), index=0)
